...
final public class OpticalComponent {
	...
	protected void setObjectSize(int function) {
		...
		if (function == Common.OBJECT_REFLECTOR
			...
			double radius = (values.leftCurvature == Common.CURVATURE_PARABOLIC || values.rightCurvature == Common.CURVATURE_PARABOLIC)?1000:10000;
			values.leftSphereRadius = radius;
			values.rightSphereRadius = radius;
			values.thickness = .1;
		}
	}
	...
	protected void reconfigure() {
		// this is the opposie end of the line used in inside-lens calculations
		farFarAway = new Vector(1e6, 1e6);
		limitPositions();
		angleRadians = -values.angle * Common.radians;
		...
		leftCenter = sqrt(values.leftSphereRadius * values.leftSphereRadius
				- values.lensRadius * values.lensRadius);
		rightCenter = sqrt(values.rightSphereRadius * values.rightSphereRadius
				- values.lensRadius * values.lensRadius);

		int leftSign = (values.leftSphereRadius < 0) ? -1 : 1;
		int rightSign = (values.rightSphereRadius < 0) ? -1 : 1;
		// prevent lens sides from crossing
		internalThickness = values.thickness;
		double leftSurface = (abs(values.leftSphereRadius) - leftCenter)
				* leftSign;
		double rightSurface = (abs(values.rightSphereRadius) - rightCenter)
				* -rightSign;
		leftSurface += values.thickness;
		rightSurface -= values.thickness;
		...
		internalThickness = max(internalThickness,parent.programValues.surfEpsilon);
		parent.internalThicknessTextField.setText(parent
				.formatNum(internalThickness));
	...
	boolean inside(Vector p1, ArrayList<Vector> polygon, double epsilon) {
		...
		for (Vector p : polygon) {
			if (op != null) {
				intersect = Common.linesIntersect(farFarAway.x, farFarAway.y, p1.x,
						p1.y, op.x, op.y, p.x, p.y);
	...
	ArrayList<Vector> createObjectPerimeter(int segments, double epsilon) {
		...
		for (int j = 0; j <= 1; j++) {
			...
			for (int i = 0; i <= segments; i++) {
				...
				Vector ca = new Vector().rotate(p.x, y, angleRadians).translate(values.xPos, values.yPos);
				//ca.rotate(p.x, y, angleRadians);
				//ca.translate(values.xPos, values.yPos);
				Vector cb = new Vector().rotate(p.y, y, angleRadians).translate(values.xPos, values.yPos);
	...
	protected void readObjectControls() {
		//parent.p("readObjectControls1: value: " + values.xPos);
		for (ControlManager cm : parent.objectControlList.values()) {
			String tag = cm.getTag();
			String value = cm.getValue();
			values.setOneValue(tag,value);
		}
		//parent.p("readObjectControls2: value: " + values.xPos);
		reconfigure();
		parent.enableComponentControls(true);
	...
	Vector profile(boolean leftSide, double ar) {
		...
		Vector v = new Vector(x - values.xPos, 0).rotate(ar).translate(values.xPos, values.yPos);
		//v.rotate(ar);
	...
	double tangent(boolean leftSide, boolean entering, Vector p, double ar, boolean reflector) {
		...
		//parent.p("tangentA: left: " + leftSide + ", angle: " + ar * Common.degrees);
		double dx = getElement(leftSide).lensProfileDXforY(this, leftSide,entering,
				 rp.y);
