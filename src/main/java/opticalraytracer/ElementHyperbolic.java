...
final public class ElementHyperbolic implements ElementBase {
	...
	public void intersections(OpticalComponent oc, boolean leftSide,
		...
		Vector thr = new Vector(-oc.signedThickness(leftSide), 0).rotate(oc.angleRadians());
		Vector tc = new Vector(oc.xPos(), oc.yPos()).translate(thr);
		Vector p1 = new Vector(op1).translateSub(tc).rotate(-oc.angleRadians()).scale(1 / signedScale, 1 / lensRadius);
		Vector p2 = new Vector(op2).translateSub(tc).rotate(-oc.angleRadians()).scale(1 / signedScale, 1 / lensRadius);
		...
		if (abs(p2.x - p1.x) < epsilon) {
			p2.x += epsilon;
		}

		if (abs(p2.y * p2.y - p1.y * p1.y) < epsilon) {
			p2.y += epsilon;
		}
		...
		/*
		 * pa0,pa1 = first hyperbolic intersection
		 * 
		 * pa2,pa3 = second hyperbolic
		 * 
		 * intersection source:
		 * ../python_equation_generator/new_hyperbolic_equation_solutions.py
		 * equation_solutions.py full intersection solutions generated by sympy
		 * based on a simple and classic hyperbolic equation pair, modified for
		 * this program:
		 * 
		 * exp1 = (x+x_1)**2 - (m*(y+y_1))**2 - z
		 * 
		 * exp2 = s*x-y
		 * 
		 * s is separately computed and submitted as an argument to the
		 * functions:
		 * 
		 * s = (y_2-y_1)/(x_2-x_1)
		 * 
		 * The modifications to the classic hyperbolic form are:
		 * 
		 * x_1,y_1 = a way to reposition the line intersection point
		 * 
		 * m controls the curve's shape independently of z
		 * 
		 * z is the location in the Z dimension of the intersecting plane
		 * 
		 * Just in case the original Python is lost, and using the sympy
		 * library:
		 * 
		 * exp1 = (x+x_1)**2 - (m*(y+y_1))**2 - z
		 * 
		...
		for (Vector p : points) {
			// parent.p("" + p);
			p.assign( p.scale(signedScale, lensRadius).rotate(oc.angleRadians()).translate(tc));
		}
	...
	public double lensProfileDXforY(OpticalComponent oc, boolean leftSide,boolean entering, double y) {
		...
		double dx = pd(y *invLrValue, zValue, mValue, 0)
				* oc.scale(leftSide) * oc.radiusSign(leftSide) * invLrValue;
		if(leftSide) {
			dx = -dx;
		}
