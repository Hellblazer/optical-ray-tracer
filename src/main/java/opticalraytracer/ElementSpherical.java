...
final public class ElementSpherical implements ElementBase {
	...
	public void intersections(OpticalComponent oc, boolean leftSide,
		...
		//Complex p1 = new Complex(op1);
		//Complex p2 = new Complex(op2);
		...
		Vector p1 = new Vector(op1).translateSub(tc).rotate(-oc.angleRadians());
		Vector p2 = new Vector(op2).translateSub(tc).rotate(-oc.angleRadians());
		...
		for (Vector p : points) {
			...
			p.assign(p.rotate(oc.angleRadians).translate(tc));
		}
	}
